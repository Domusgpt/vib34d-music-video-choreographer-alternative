export class DynamicParameterBridge {
    ...
    registerParameter(name, definition = {}) {
        ...
        this.parameterRegistry.set(name, { ...existing, ...definition });
    }
    ...
    _getCurrentParameterValue(name) {
        ...
        try {
            if (engine.parameterManager && engine.parameterManager.getParameterValue) {
    ...
    _setEngineParameter(name, value, descriptor) {
        ...
        const applyToEngine = (eng) => {
            ...
            if (eng.parameterManager && eng.parameterManager.setParameter) {
                try {
                    eng.parameterManager.setParameter(name, value);
                    return true;
                } catch (err) {
                    console.warn('[DynamicParameterBridge] parameterManager.setParameter failed', name, err);
                }
            }
