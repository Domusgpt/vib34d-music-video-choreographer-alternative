...
const DEFAULT_VARIATION_TARGET = 30;
const CUSTOM_VARIATION_SLOTS = 70;
const BASE_GEOMETRY_CLASS_MAP = {
...
export class VariationManager {
    constructor(engine) {
        this.engine = engine;
        this.defaultVariationTarget = DEFAULT_VARIATION_TARGET;
    ...
    handleGeometryUpdate(names) {
        const resolvedNames = Array.isArray(names) && names.length
            ? names
            : GeometryLibrary.getGeometryNames();

        this.geometryMetadata = this.buildGeometryMetadata(resolvedNames);
        this.defaultVariations = this.buildDefaultVariationDefinitions(this.geometryMetadata);
        this.updateEngineVariationMetadata();
        this.syncCurrentVariationIndex();
        this.refreshGridIfMounted();
    }

    buildGeometryMetadata(names = []) {
        const seen = new Set();
        return names
            .map(name => GeometryLibrary.normalizeName(name))
            .filter(Boolean)
            .map((normalized, index) => {
                ...
                return {
                    key,
                    displayName: this.toDisplayName(normalized),
                    cssClass: BASE_GEOMETRY_CLASS_MAP[key] || this.toCssClass(key, index),
                    levels: this.getLevelCountForGeometry(key)
                };
            })
            .filter(Boolean);
    }
    ...
    buildDefaultVariationDefinitions(metadata) {
        if (!metadata.length) {
            return [];
        }

        const perGeometry = metadata.map((meta, geometryIndex) => {
        ...
        while (definitions.length < this.defaultVariationTarget && added) {
            added = false;
            perGeometry.forEach(entries => {
                if (definitions.length >= this.defaultVariationTarget) {
                    return;
                }
    ...
    getGeometryDisplayName(index) {
        ...
        return normalized ? this.toDisplayName(normalized) : 'Unknown';
    }
