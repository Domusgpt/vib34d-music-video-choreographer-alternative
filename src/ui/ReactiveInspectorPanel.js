...
export function installReactiveInspectorPanel({
    ...
    const restoredFromStorage = ReactiveVisualizerInspector.loadStateFromStorage({ merge: true });
    ...
    settingsRow.style.gap = '6px';
    settingsRow.style.marginTop = '10px';

    const saveSettingsBtn = makeButton('Save Settings');
    const loadSettingsBtn = makeButton('Load Settings');
    const exportSettingsBtn = makeButton('Export JSON');
    const importSettingsBtn = makeButton('Import JSON');
    const resetSettingsBtn = makeButton('Reset Defaults', {
        baseBg: 'rgba(82, 21, 21, 0.45)',
        hoverBg: 'rgba(110, 35, 35, 0.75)',
        baseBorder: 'rgba(255, 149, 128, 0.6)',
        hoverBorder: 'rgba(255, 174, 150, 0.85)'
    });

    [
        [saveSettingsBtn, 'Save Settings'],
        [loadSettingsBtn, 'Load Settings'],
        [exportSettingsBtn, 'Export JSON'],
        [importSettingsBtn, 'Import JSON'],
    ...
    settingsRow.append(saveSettingsBtn, loadSettingsBtn, exportSettingsBtn, importSettingsBtn, resetSettingsBtn);
    ...
    continuityDetails.append(continuitySummaryEl, presetRow, thresholdGrid, continuityHint, settingsRow);
    ...
    panel.append(header, controls, continuityDetails, content, footer, importFileInput);
    ...
    const settingsResetTimers = new Map();

    const setTemporaryButtonLabel = (button, nextLabel, duration = 1600) => {
        const defaultLabel = button.dataset.defaultLabel || button.textContent;
        button.dataset.defaultLabel = defaultLabel;
        button.textContent = nextLabel;
        if (settingsResetTimers.has(button)) {
            clearTimeout(settingsResetTimers.get(button));
        }
        const timeout = setTimeout(() => {
            button.textContent = button.dataset.defaultLabel;
            settingsResetTimers.delete(button);
        }, duration);
        settingsResetTimers.set(button, timeout);
    };
    ...
    const teardown = () => {
        ...
        settingsResetTimers.forEach((timeout) => clearTimeout(timeout));
        stopSessionTicker();
    ...
    saveSettingsBtn.addEventListener('click', () => {
        const success = ReactiveVisualizerInspector.persistStateToStorage();
        if (success) {
            setTemporaryButtonLabel(saveSettingsBtn, 'Saved!');
        } else {
            setTemporaryButtonLabel(saveSettingsBtn, 'Storage Unavailable', 2000);
        }
    });

    loadSettingsBtn.addEventListener('click', () => {
        const loaded = ReactiveVisualizerInspector.loadStateFromStorage({ merge: false });
        if (loaded) {
            ...
            setTemporaryButtonLabel(loadSettingsBtn, 'Loaded!');
        } else {
            setTemporaryButtonLabel(loadSettingsBtn, 'No Saved State', 2000);
        }
    });

    exportSettingsBtn.addEventListener('click', async () => {
        const payload = ReactiveVisualizerInspector.exportState({ pretty: true });
        let exported = false;
        if (navigator?.clipboard?.writeText) {
            try {
                await navigator.clipboard.writeText(payload);
                exported = true;
                setTemporaryButtonLabel(exportSettingsBtn, 'JSON Copied!', 2000);
            } catch (error) {
                console.warn('ReactiveInspectorPanel settings clipboard write failed:', error);
            }
        }
        if (!exported) {
            const blob = new Blob([payload], { type: 'application/json' });
            ...
            const tempLink = document.createElement('a');
            tempLink.href = url;
            tempLink.download = 'reactive-inspector-settings.json';
            document.body.appendChild(tempLink);
            tempLink.click();
            tempLink.remove();
            URL.revokeObjectURL(url);
            exported = true;
            setTemporaryButtonLabel(exportSettingsBtn, 'JSON Saved!', 2000);
        }
    });
    ...
    importFileInput.addEventListener('change', async (event) => {
        ...
        try {
            ...
            const applied = ReactiveVisualizerInspector.importState(text, { merge: false, persist: true });
            if (applied) {
                syncContinuityInputs();
                renderContinuity(latestSummary);
                if (!isPaused) {
                ...
                setTemporaryButtonLabel(importSettingsBtn, 'Imported!', 2000);
            } else {
                setTemporaryButtonLabel(importSettingsBtn, 'Invalid JSON', 2200);
            }
        } catch (error) {
            console.warn('ReactiveInspectorPanel settings import failed:', error);
            setTemporaryButtonLabel(importSettingsBtn, 'Import Failed', 2200);
        } finally {
    ...
    resetSettingsBtn.addEventListener('click', () => {
        ReactiveVisualizerInspector.resetState({ clearProfiles: true, clearStorage: true });
        syncContinuityInputs();
        renderContinuity(latestSummary);
        if (!isPaused) {
            renderRolling(currentSampleSize);
        }
        setTemporaryButtonLabel(resetSettingsBtn, 'Defaults Restored', 2200);
    });
    ...
    if (restoredFromStorage) {
        setTemporaryButtonLabel(loadSettingsBtn, 'Loaded from Browser', 2200);
    }
