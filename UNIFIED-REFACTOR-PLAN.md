...
3. **Unified System Architecture (in progress)**:
   ...
   - ✅ Timeline editor sequences now source defaults + form controls from shared metadata with sanitized rendering
   - ✅ Choreography import/export captures timeline defaults + geometry snapshots so shared metadata survives round-trips
   - 🔜 Port remaining consumers (gallery, remote embeds) to query registry instead of globals
...
4. **System-aware interface foundation** ✅
   ...
   - Adaptive control panel renders metadata-driven parameter groups with shared accent theming and hosts the integrated timeline shell
   - Timeline presets, quick actions, and editor toggles now resolve through shared metadata with live registry updates
...
**Next Command**: Migrate gallery + remote embeds onto the registry-aware timeline loader and verify exported choreography metadata restores cleanly across faceted and holographic presets.
