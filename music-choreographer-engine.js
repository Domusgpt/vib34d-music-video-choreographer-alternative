...
export class MusicVideoChoreographer {
    constructor(mode = 'reactive') {
        ...
        this.domRefs = {};
        this.eventHandlers = {};
        this.boundLoadAIChoreography = (data) => this.ingestAIChoreography(data);
    ...
    applyGeometryMetadataToUI() {
        ...
        if (!this.domRefs.geometrySlider) {
            this.domRefs.geometrySlider = document.getElementById('geometry');
        }

        const slider = this.domRefs.geometrySlider;
        if (slider) {
    ...
    destroy() {
        ...
        if (this.isPlaying) {
            try {
                ...
                console.warn('[MusicVideoChoreographer] stop during destroy failed', err);
            }
        }
        ...
        if (this.audio) {
            try {
                ...
                console.warn('[MusicVideoChoreographer] audio cleanup failed', err);
            }
        }
        ...
        if (this.sourceNode) {
            try {
                ...
                console.warn('[MusicVideoChoreographer] source node disconnect failed', err);
            }
        ...
        if (this.analyser) {
            try {
                ...
                console.warn('[MusicVideoChoreographer] analyser disconnect failed', err);
            }
            this.analyser = null;
        }
        ...
        if (ctx && typeof ctx.close === 'function') {
            ctx.close().catch(err => {
                console.warn('[MusicVideoChoreographer] audio context close failed', err);
            });
        }
        ...
        if (this.currentEngine && typeof this.currentEngine.destroy === 'function') {
            try {
                ...
                console.warn('[MusicVideoChoreographer] engine destroy failed', err);
            }
        }
        this.currentEngine = null;
        this.canvasManager = null;

        if (typeof window !== 'undefined' && window.loadAIChoreography === this.boundLoadAIChoreography) {
            window.loadAIChoreography = undefined;
        }
    ...
    updateGeometryReadout(index) {
        ...
        if (!this.domRefs.geometryReadout) {
            this.domRefs.geometryReadout = document.getElementById('v-geometry');
        }

        const readout = this.domRefs.geometryReadout;
        if (!readout) return;
    ...
    async init() {
        ...
        if (typeof window !== 'undefined') {
            window.loadAIChoreography = this.boundLoadAIChoreography;
        }
    ...
    cacheDomElements() {
        ...
        this.domRefs = {
            ...
            layersContainer: document.getElementById('vib34dLayers'),
            geometrySlider: this.domRefs.geometrySlider || document.getElementById('geometry'),
            geometryReadout: this.domRefs.geometryReadout || document.getElementById('v-geometry')
        };
    }
    ...
    removeEventListeners() {
        ...
        if (refs.fileInput && handlers.onFileChange) {
            refs.fileInput.removeEventListener('change', handlers.onFileChange);
        }
        if (refs.playButton && handlers.onPlay) {
            refs.playButton.removeEventListener('click', handlers.onPlay);
        }
        if (refs.pauseButton && handlers.onPause) {
            refs.pauseButton.removeEventListener('click', handlers.onPause);
        }
        if (refs.stopButton && handlers.onStop) {
            refs.stopButton.removeEventListener('click', handlers.onStop);
        }
        if (refs.timeline && handlers.onTimelineClick) {
            refs.timeline.removeEventListener('click', handlers.onTimelineClick);
        }
        if (Array.isArray(refs.systemButtons) && handlers.onSystemClick) {
            refs.systemButtons.forEach(btn => btn.removeEventListener('click', handlers.onSystemClick));
        }
        if (this.audio && handlers.onAudioEnded) {
            this.audio.removeEventListener('ended', handlers.onAudioEnded);
        }
        if (this.audio && handlers.onAudioTimeUpdate) {
            this.audio.removeEventListener('timeupdate', handlers.onAudioTimeUpdate);
        }
    ...
    setupEventListeners() {
        ...
        this.eventHandlers = {
            ...
            onTimelineClick: (e) => {
                ...
                const offset = (e.clientX - rect.left) / width;
            ...
            onAudioEnded: () => this.stop(),
            onAudioTimeUpdate: () => this.updateTimeline()
        };

        const refs = this.domRefs;

        if (refs.fileInput) {
            refs.fileInput.addEventListener('change', this.eventHandlers.onFileChange);
        }
        if (refs.playButton) {
            refs.playButton.addEventListener('click', this.eventHandlers.onPlay);
        }
        if (refs.pauseButton) {
            refs.pauseButton.addEventListener('click', this.eventHandlers.onPause);
        }
        if (refs.stopButton) {
            refs.stopButton.addEventListener('click', this.eventHandlers.onStop);
        }
        if (refs.timeline) {
            refs.timeline.addEventListener('click', this.eventHandlers.onTimelineClick);
        }
        if (Array.isArray(refs.systemButtons)) {
            refs.systemButtons.forEach(btn => btn.addEventListener('click', this.eventHandlers.onSystemClick));
        }
        if (this.audio) {
            this.audio.addEventListener('ended', this.eventHandlers.onAudioEnded);
            this.audio.addEventListener('timeupdate', this.eventHandlers.onAudioTimeUpdate);
        }
    }
    ...
    async loadAudioFile(file) {
        ...
        // Enable controls
        this.cacheDomElements();
        if (this.domRefs.playButton) this.domRefs.playButton.disabled = false;
        if (this.domRefs.pauseButton) this.domRefs.pauseButton.disabled = false;
        if (this.domRefs.stopButton) this.domRefs.stopButton.disabled = false;
    ...
    renderSequenceList() {
        if (!this.domRefs.sequenceList) {
            this.domRefs.sequenceList = document.getElementById('sequence-list');
        }
        const list = this.domRefs.sequenceList;
        if (!list) return;
    ...
    async switchSystem(systemName) {
        ...
        // Clear canvases
        if (!this.domRefs.layersContainer) {
            this.domRefs.layersContainer = document.getElementById('vib34dLayers');
        }
        const container = this.domRefs.layersContainer;
        if (!container) {
            console.warn('No layer container found for system switch');
            return;
        }
        container.innerHTML = '';
    ...
    onBeat() {
        ...
        if (!this.domRefs.beatIndicator) {
            this.domRefs.beatIndicator = document.getElementById('beatIndicator');
        }
        const indicator = this.domRefs.beatIndicator;
        if (indicator) {
    ...
    updateTimeline() {
        ...
        if (!this.domRefs.timelineProgress) {
            this.domRefs.timelineProgress = document.getElementById('timeline-progress');
        }

        const progressEl = this.domRefs.timelineProgress;
    ...
    updateInfoPanel(audioData) {
        if (!this.domRefs.beatInfo) {
            this.domRefs.beatInfo = document.getElementById('beat-info');
        }
        if (!this.domRefs.energyInfo) {
            this.domRefs.energyInfo = document.getElementById('energy-info');
        }

        if (this.domRefs.beatInfo) {
            this.domRefs.beatInfo.textContent = `BPM: ${this.detectedBPM} | Threshold: ${this.beatThreshold}`;
        }
        if (this.domRefs.energyInfo) {
            this.domRefs.energyInfo.textContent = `Energy: ${(audioData.energy * 100).toFixed(0)}% | Bass: ${(audioData.bass * 100).toFixed(0)}%`;
        }
    }

    updateStatus(message) {
        if (!this.domRefs.status) {
            this.domRefs.status = document.getElementById('status');
        }
        if (this.domRefs.status) {
            this.domRefs.status.textContent = message;
        }
    }
