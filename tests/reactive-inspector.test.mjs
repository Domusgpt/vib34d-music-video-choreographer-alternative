...
ReactiveVisualizerInspector.resetState({ clearProfiles: true });

const baseSnapshot = ReactiveVisualizerInspector.getStateSnapshot();
assert.equal(baseSnapshot.version, ReactiveVisualizerInspector.storageVersion, 'State snapshot should include a version identifier');
assert.equal(baseSnapshot.historyLimit, ReactiveVisualizerInspector.historyLimit, 'Snapshot history limit should match inspector state');
assert.equal(baseSnapshot.thresholds.rotationJitterThreshold, defaultContinuity.rotationJitterThreshold, 'Snapshot thresholds should default to baseline values');

ReactiveVisualizerInspector.setHistoryLimit(12);
ReactiveVisualizerInspector.throttleMs = 420;
ReactiveVisualizerInspector.updateContinuityThresholds({ rotationJitterThreshold: 0.72, intervalWarningMs: 2400 });
ReactiveVisualizerInspector.updateContinuityThresholds({ rotationJitterThreshold: 0.9 }, { visualizer: 'holo-test' });

const tunedSnapshot = ReactiveVisualizerInspector.getStateSnapshot();
assert.equal(tunedSnapshot.historyLimit, 12, 'Snapshot should capture updated history limit');
assert.equal(tunedSnapshot.throttleMs, 420, 'Snapshot should capture throttle overrides');
assert.equal(Number(tunedSnapshot.thresholds.rotationJitterThreshold.toFixed(2)), 0.72, 'Snapshot should include updated global thresholds');
assert.equal(Number(tunedSnapshot.continuityProfiles['holo-test'].rotationJitterThreshold.toFixed(2)), 0.9, 'Snapshot should include custom continuity profiles');

ReactiveVisualizerInspector.resetState({ clearProfiles: true });
const resetSnapshot = ReactiveVisualizerInspector.getStateSnapshot();
assert.equal(resetSnapshot.historyLimit, baseSnapshot.historyLimit, 'Reset state should restore the base history limit');
assert.equal(resetSnapshot.throttleMs, baseSnapshot.throttleMs, 'Reset state should restore throttle defaults');
assert.equal(Object.keys(resetSnapshot.continuityProfiles).length, 0, 'Reset state should clear continuity profiles');

const imported = ReactiveVisualizerInspector.importState({
    thresholds: {
        rotationJitterThreshold: 0.61,
        audioSpikeThreshold: 0.41,
        intervalWarningMs: 2100
    },
    continuityProfiles: {
        'quantum-viz': { rotationJitterThreshold: 0.95 }
    },
    historyLimit: 9,
    throttleMs: 333
}, { merge: false, persist: false });

assert.equal(imported, true, 'Importing structured state should succeed');
const importedConfig = ReactiveVisualizerInspector.getContinuityConfig();
assert.equal(Number(importedConfig.rotationJitterThreshold.toFixed(2)), 0.61, 'Imported thresholds should apply to the inspector');
assert.equal(ReactiveVisualizerInspector.historyLimit, 9, 'Imported history limit should update inspector state');
assert.equal(ReactiveVisualizerInspector.throttleMs, 333, 'Imported throttle setting should update inspector state');
const importedProfileInfo = ReactiveVisualizerInspector.getContinuityProfileInfo('quantum-viz');
assert.equal(importedProfileInfo.profile, 'quantum-viz', 'Imported continuity profile should be registered');

assert.equal(ReactiveVisualizerInspector.importState('not-json'), false, 'Importing invalid JSON should fail gracefully');
const exportedState = ReactiveVisualizerInspector.exportState({ pretty: true });
assert.equal(typeof exportedState, 'string', 'Export should return a JSON string');
assert(exportedState.includes('"thresholds"'), 'Exported JSON should include threshold data');
...
const persisted = ReactiveVisualizerInspector.persistStateToStorage();
assert.equal(persisted, true, 'Persisting state should succeed when localStorage is available');
ReactiveVisualizerInspector.updateContinuityThresholds({ rotationJitterThreshold: 0.77 });
ReactiveVisualizerInspector.persistStateToStorage();
const storedPayload = global.window.localStorage.getItem(ReactiveVisualizerInspector.storageKey);
assert(storedPayload, 'Persisted payload should exist in localStorage');
const parsedStored = JSON.parse(storedPayload);
assert.equal(Number(parsedStored.thresholds.rotationJitterThreshold.toFixed(2)), 0.77, 'Persisted payload should capture the latest thresholds');

ReactiveVisualizerInspector.updateContinuityThresholds({ rotationJitterThreshold: 0.44 }, { persistProfile: false });
const loadedFromStorage = ReactiveVisualizerInspector.loadStateFromStorage({ merge: false });
assert.equal(loadedFromStorage, true, 'Loading from storage should apply persisted state');
const loadedConfig = ReactiveVisualizerInspector.getContinuityConfig();
assert.equal(Number(loadedConfig.rotationJitterThreshold.toFixed(2)), 0.77, 'Loaded state should restore persisted thresholds');

const cleared = ReactiveVisualizerInspector.clearStateFromStorage();
assert.equal(cleared, true, 'Clearing persisted state should succeed');
assert.equal(global.window.localStorage.getItem(ReactiveVisualizerInspector.storageKey), null, 'Clearing persisted state should remove the saved payload');
